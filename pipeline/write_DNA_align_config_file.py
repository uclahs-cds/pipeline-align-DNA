...
if __name__ == '__main__':
	...
	string_for_writing = "final String SAMPLE = '" + sample_name + "'\n" \
		+ 'params {' + '\n' \
		+ '\t' + 'sample_name = SAMPLE' + '\n' \
		+ '\t' + 'input_csv = "' + os.path.dirname(args['<input_file>']) + '${SAMPLE}.csv' + '"\n' \
		+ '\t' + aligner_type_choice(args) + '\n' \
		+ '\t' + 'output_dir = "' + os.path.join(args['<output_dir>'], '${SAMPLE}') + '"\n' \
		+ '\t' + 'temp_dir = "' + args['<temp_dir>'] + '"\n' \
		+ '\t' + is_save_inter(args) + '\n' \
		+ '\t' + is_cache_intermediate_pipeline_steps(args) + '\n' \
		+ '\t' + is_blcds_registered_dataset_input(args) + '\n' \
		+ '\t' + blcds_registered_dataset_output(args) + '\n' \
		+ is_save_bam_and_log_to_blcds(args) \

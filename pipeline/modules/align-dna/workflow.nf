...
include { Align_bwa_mem2 } from './fastq2bam.nf'
include { Align_hisat2 } from './fastq2bam.nf'
...
workflow aligndna {
   ...
   main:
      if (params.bwa_mem2) {
         ...
         PicardTools_SortSam(Align_bwa_mem2.out.bam)
      ...
      else if (params.hisat2) { 
         Align_hisat2(
            ich_samples,
            ich_reference_fasta,
            ich_reference_index_files.collect()
            )	 
         PicardTools_SortSam(Align_hisat2.out.bam)
         PicardTools_MarkDuplicates(PicardTools_SortSam.out.bam.collect())
         PicardTools_BuildBamIndex(PicardTools_MarkDuplicates.out.bam)
         Generate_Sha512sum(PicardTools_BuildBamIndex.out.bai.mix(PicardTools_MarkDuplicates.out.bam))
         validate_file(

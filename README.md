...
| `reference_fasta_bwa` | BWA-MEM2 | path | Absolute path to the reference genome `fasta` file. The reference genome is used by BWA-MEM2 for alignment. |
| `reference_fasta_index_files_bwa` | BWA-MEM2 | path | Absolute path to the genome index with a pattern matching. The index must be generated by the `bwa-mem2 index` command using the correct version of BWA-MEM2. |
| `reference_fasta_hisat2` | HISAT2 | path | Absolute path to the reference genome `fasta` file. The reference genome is used by HISAT2 for alignment. |
| `reference_fasta_index_prefix` | HISAT2 | path | Absolute path up to the genome index basename. The index must be generated by the `hisat2-build` command. |

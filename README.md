...
| `reference_fasta_dict` | yes | path | The dictionary of the reference fasta. This is the `.dict` file generated by `bwa-mem2 index`. This file name can **not** be matched by the pattern of `reference_fasta_index_files`. The recommended way is to create a soft link of genome.dict -> genome.fa.dict. |
...
| `blcds_cluster_slurm` | no | boolean | Pipeline is to run on the Slurm cluster. Set to `false` if it is to run on the SGE cluster. This is used only when `blcds_registered_dataset_output = true` and `blcds_registered_dataset_input = false` |
